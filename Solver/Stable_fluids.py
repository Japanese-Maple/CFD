...
def force(t, X, Y, a):
    ...
    region = ((X > H/2-1.5) & (X < H/2+0.5) &
              (Y > H/2-0.5) & (Y < H/2+0.5))
    
    # force_field[region, 0] = -13*np.sin(Y[region]*decay)
    # force_field[region, 1] = 13*np.cos(X[region]*decay)
...
def make_circular_obstacle(X, Y, center, radius):
    cx, cy = center
    distance = np.sqrt((X - cx)**2 + (Y - cy)**2)
    mask = distance < radius
    return mask

def apply_obstacle(field, mask):
    field = field.copy()
    field[mask, :] = 0 
    return field
